...
def evaluate_svc_scalability(dataset_dir, dimensions, sample_sizes, eta=0.01, n_iter=50, C=1.0):
    ...
    for d in dimensions:
        for n in sample_sizes:
            ...
            # Dual problem
            model = LinearSVC(penalty='l2', C=C, random_state=1, loss='hinge', max_iter=n, dual=True)
            
            # Fails to produce results? Rerun later.
            # model = LinearSVC(penalty='l2', C=C, random_state=1, loss='hinge', dual=True)

            # Primal problem
            # model = LinearSVC(C=C, random_state=1,  max_iter=n, dual=False)
            
            # This may fail.
            # model = LinearSVC(C=C, random_state=1, dual=False)
